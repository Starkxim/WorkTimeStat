...
def merge_files(money_file: str, rest_file: str, save_directory: str) -> str:
    ...
    public_holidays = pd.read_csv(f"HolidayData/public_holidays_{year}.csv")[
        "date"
    ].tolist()
    makeup_workdays = pd.read_csv(f"HolidayData/makeup_workdays_{year}.csv")[
        "date"
    ].tolist()
    ...
    df_money = df_money[
        ["申请人", "开始时间", "结束时间", "时长", "项目编号", "当前审批状态"]
    ]
    df_rest = df_rest[
        ["申请人", "开始时间", "结束时间", "加班时长", "项目编号", "当前审批状态"]
    ]
    ...
    df_combined["时长"] = df_combined["时长"].apply(
        lambda x: float(str(x).replace("小时", "")) if isinstance(x, str) else x
    )

    project_money_hours = (
        df_combined[df_combined["类型"] == "计薪"]
        .groupby(["申请人", "项目编号"])["时长"]
        .sum()
        .reset_index()
    )
    project_rest_hours = (
        df_combined[df_combined["类型"] == "调休"]
        .groupby(["申请人", "项目编号"])["时长"]
        .sum()
        .reset_index()
    )
    ...
    df_combined = pd.merge(
        df_combined, project_money_hours, on=["申请人", "项目编号"], how="left"
    )
    df_combined = pd.merge(
        df_combined, project_rest_hours, on=["申请人", "项目编号"], how="left"
    )
    ...
    if "类型" in df_combined.columns:
        df_combined["项目调休总时长"] = df_combined.apply(
            lambda x: 0 if x["类型"] == "计薪" else x["项目调休总时长"], axis=1
        )
        df_combined["项目计薪总时长"] = df_combined.apply(
            lambda x: 0 if x["类型"] == "调休" else x["项目计薪总时长"], axis=1
        )
    ...
    df_combined["项目计薪总时长"] = df_combined["项目计薪总时长"].apply(
        lambda x: f"{x}小时" if x > 0 else ""
    )
    df_combined["项目调休总时长"] = df_combined["项目调休总时长"].apply(
        lambda x: f"{x}小时" if x > 0 else ""
    )
    df_combined["计薪总时长"] = df_combined["计薪总时长"].apply(
        lambda x: f"{x}小时" if x > 0 else ""
    )
    df_combined["调休总时长"] = df_combined["调休总时长"].apply(
        lambda x: f"{x}小时" if x > 0 else ""
    )
    ...
    try:
        ...
        raise Exception(f"保存文件时出现错误: {e}，请检查文件是否被占用")
    ...
    def merge_cells(ws, col: int):
        ...
        for row in range(3, max_row + 2):
            if (
                ...
                or (
                    col in [7, 8]
                    and ws.cell(row=row, column=6).value
                ...
                if merge_start != row - 1:
                    ...
                    ws.cell(row=merge_start, column=col).alignment = Alignment(
                        vertical="center"
                    )
                merge_start = row
