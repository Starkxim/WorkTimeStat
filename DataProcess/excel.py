...
def merge_files(money_file, rest_file, save_directory):
    ...
    def merge_cells(ws, col):
        ...
        for row in range(3, max_row + 2):
            if ws.cell(row=row, column=col).value != ws.cell(row=row - 1, column=col).value or ws.cell(row=row, column=1).value != ws.cell(row=row - 1, column=1).value or (col in [7, 8] and ws.cell(row=row, column=6).value != ws.cell(row=row - 1, column=6).value):
