...
def create_main_window():
    ...
    additional_menu.add_command(label="更新节假日和补班日期", command=uf.Update_holidays)

    # 创建“统计”菜单
    stats_menu = Menu(menu, tearoff=False)
    menu.add_cascade(label="统计", menu=stats_menu)
    stats_menu.add_command(label="每月加班时长", command=lambda: uf.show_monthly_overtime(tree))
    stats_menu.add_command(label="加班时长图表", command=lambda: uf.show_overtime_chart(tree))

    # 创建“文件”菜单
    file_menu = Menu(menu, tearoff=False)
    menu.add_cascade(label="文件", menu=file_menu)
    file_menu.add_command(label="选择计薪加班文件", command=lambda: uf.select_money_file(tree))
    file_menu.add_command(label="选择调休加班文件", command=lambda: uf.select_rest_file(tree))
    ...
    search_button = ttk.Button(search_frame, text="搜索", command=lambda: uf.search_tree(tree, search_entry.get()))
